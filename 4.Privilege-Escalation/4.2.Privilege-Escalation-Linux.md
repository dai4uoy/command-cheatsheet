...
./pspy64 -i 20 -c 2>/dev/null | grep -vaiE 'docker|containerd|runc|dockerd|iptables|/pause|kworker|systemd-(udev|journal|resolve|timesyn|logind|network|oom)|sd-pam|snapd|packagekit|polkit|gvfs|pspy64|apport' Clean stream (strip common background noise, keep everything else):
```
